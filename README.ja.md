...
|---|---|---|
| **Windows** | ✅ インストーラーに同梱 | RTL-SDR、HackRF One、Airspy、Airspy HF+、ADALM-Pluto |
...
ADALM-Pluto は USB 接続（Zadig で WinUSB ドライバー適用が必要）またはネットワーク接続（192.168.2.1、追加ドライバー不要）で使用できます。
...
| ADALM-Pluto | SDR | ✓ 同梱† | — | SoapyPlutoSDR、スペクトラム・復調 |
...
† ADALM-Pluto：Windows 同梱（SoapyPlutoSDR + libiio）。USB 接続は Zadig で WinUSB ドライバー適用が必要、ネットワーク接続（192.168.2.1）はドライバー不要。実機での動作未確認。
