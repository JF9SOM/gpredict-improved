...
- **GitHub Actions: `make_latest: true`**（`prerelease: true` を廃止）。3プラットフォーム全ビルドジョブで設定済み。最新リリース: `v0.1.0-beta.33`
...
### SDR・デジタルモード
4. **SDR機能の追加（フェーズ1: 初期実装）**
   - `src/sdr/` バックエンド（device・pipeline）
   - Rig Settings に SDR Settings タブ追加（SoapySDR デバイス設定・Rig 1/2 割当）
   - SdrRigAdapter（既存 RigController を継承）
   - SDR Control タブ（スペクトラム表示 → NFM/SSB/CW デモジュレーター → IQ 録音）
   - Help メニューに SDR Device Installation ダイアログ
   - **設計方針は「SDR 機能設計方針」セクション参照**
...
外部ツール（SatDump・Direwolf・WSJT-X）はサブプロセス起動。内部実装しない。
