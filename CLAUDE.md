...
- ft8_lib 未インストール時は赤バナー表示・TX Enable 無効化。インストール先: `~/.local/share/gpredict-improved/ft8lib/`
...
1. ユーザーインストール版（`~/.local/share/gpredict-improved/ssdv/`）
...
ローカルで手動ビルドした `libft8.so` を `~/.local/share/gpredict-improved/ft8lib/` に配置すれば動作）。
...
- 配布: CI でソースビルドし GitHub Releases にアップロード
  - `ft8lib-linux-x86_64.tar.gz` / `ft8lib-windows-x86_64.zip` / `ft8lib-macos-arm64.tar.gz`
- `Help > ft8lib Installation…` ダイアログ: 未インストール時の案内
- ユーザーインストール先:
  ```
  Linux:   ~/.local/share/gpredict-improved/ft8lib/
  macOS:   ~/Library/Application Support/gpredict-improved/ft8lib/
  Windows: %APPDATA%/gpredict-improved/ft8lib/
  ```
...
mkdir -p ~/.local/share/gpredict-improved/ft8lib/
cp libft8.so ~/.local/share/gpredict-improved/ft8lib/
```
