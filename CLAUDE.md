...
- **GitHub Actions: `make_latest: true`**（`prerelease: true` を廃止）。3プラットフォーム全ビルドジョブで設定済み。最新リリース: `v0.1.0-beta.34`
...
- **実動作確認済みリグ・デバイス**（2026-06-09）
  - FTX-1F（Hamlib 4.7.1、NET Control）: ドップラー補正・VFO制御・CTCSS 動作確認済み
...
### 継続中・優先度高
1. **ドップラー補正の実動作確認** — 各種リグ（IC-9700・TS-2000・FT-817ND 等）での実衛星通信テスト（FTX-1F + RTL-SDR/HackRF は確認済み）
...
### SDR・デジタルモード
4. ~~**SDR機能の追加（フェーズ1: 初期実装）**~~ **→ beta.34 で完了**
5. **SDR フェーズ2（将来）** — HRPT/LRPT 画像・APRS・FT4・gr-satellites・AI-CW・SSTV（設計方針は「SDR 機能設計方針」セクション参照）
6. **SDR Device Installation ダイアログ**（Help メニュー）— USB VID/PID スキャン・ドライバ自動インストール誘導

### 配布・ビルド
7. **Windows・macOS beta.34 ビルドの動作確認** — CI ビルド成功後、実機での SDR 含む全機能検証

### データ・連携
8. **観測ログ機能** — 実際に追尾・通信した衛星パスを記録・集計・エクスポートする機能
9. **多言語対応（日本語）** — フェーズ2として日本語UIの追加（翻訳ファイルは準備済み）

### ハードウェア連携
10. **追加リグの実機テスト** — IC-9700・TS-2000・FT-817ND 等でのドップラー制御動作確認（satmode含む）
11. **WSJT-X / JS8Call 連携** — デジタルモード運用ソフトとの周波数・モード連動（将来）
