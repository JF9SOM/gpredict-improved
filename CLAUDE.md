...
- Direct mode split init: pyserial CI-V `16 5A 01` **before** `rig.open()` — enters SAT mode without Hamlib
...
**Root cause (historical)**: Hamlib `set_func(RIG_FUNC_SATMODE, 1)` sends CI-V `16 59 01` which is
the **Dual Watch** command, NOT the SAT mode command. IC-9100 responded `FB` (OK) but the SAT lamp
never lit. This was confirmed by comparing rigctld debug log (`subcmd=0x5A` in `icom_set_split_vfo`)
with what `set_func` actually sent (`subcmd=0x59`).

As a result, Direct mode was never in SAT mode — it was always in SPLIT mode. All previous work
(satmode-OFF toggle, Cold Start warmup, `_satmode_enter`) was addressing symptoms of the wrong
CI-V command, not real hardware behaviour.

**Correct CI-V commands for IC-9100/IC-9700 SAT mode**:
- `FE FE <civ_addr> E0 16 5A 01 FD` — SAT mode ON
- `FE FE <civ_addr> E0 16 5A 00 FD` — SAT mode OFF
- `FE FE <civ_addr> E0 16 59 xx FD` — Dual Watch (unrelated; do NOT use for satmode)

**Implementation (src/rig/controller.py)**:
- `_satmode_preopen_civ(enable)`: pyserial helper that sends `16 5A 01/00` while port is free
- `connect()`: calls `_satmode_preopen_civ(True)` **before** `rig.open()` for satmode rigs
- `satmode_warmup()`: calls `_satmode_preopen_civ(True)` at app startup (no Hamlib open/close)
- After `rig.open()`, Hamlib's `cache.satmode` stays `0` (we never used `set_func` through Hamlib),
  so `ic9700_set_vfo` does NOT reject `RIG_VFO_TX`
- `_init_split()`: just sets `_satmode_active = True` — no `set_func`, no `sleep`
- DL: `set_freq(RIG_VFO_MAIN, dl_hz)` as before
- UL (periodic): `set_freq(RIG_VFO_TX, ul_hz)` — no toggle needed, works reliably in SAT mode

**Why VFO_TX works**: in SAT mode, Hamlib maps `RIG_VFO_TX` to the TX VFO (Sub/UL).
Since `cache.satmode=0` (we entered via pyserial, not Hamlib), `ic9700_set_vfo` does not
reject the call. Confirmed with test script `scripts/test_ic9100_sat5a.py` (2026-06-20).

**Why NOT to use `set_func(RIG_FUNC_SATMODE)` for satmode rigs**:
Hamlib routes `RIG_FUNC_SATMODE` to `subcmd=0x59` (Dual Watch) for the icom backend.
The SAT mode command is `subcmd=0x5A`. This is a permanent Hamlib behaviour — do not add
`set_func(SATMODE)` calls to the codebase for IC-9100/IC-9700.
...
>   - クロスバンドUL: `set_freq(VFO_TX)` 方式（CI-V `16 5A 01` で SAT mode ON 後）で正常書き込み確認済み（2026-06-20）
...
- NET mode: `HamlibNetController._apply_ctcss_civ_direct(tone_hz)` — same pyserial approach; on Linux rigctld does not hold an exclusive lock on the serial port, so pyserial can write CI-V frames even while rigctld is running (confirmed on IC-9100). All 5 frames sent in a single `write()` call to prevent rigctld interleaving: Main TONE OFF → Sub select → tone BCD → Sub TONE ON/OFF → Main select.
...
```
FE FE <civ_addr> E0 16 5A 00 FD    # SAT MODE OFF  (state reset; 16 5A = SAT mode)
FE FE <civ_addr> E0 16 5A 01 FD    # SAT MODE ON   (re-enter satmode cleanly)
FE FE <civ_addr> E0 07 D1 FD       # Select Sub band
...
**Why the SAT MODE OFF→ON reset is needed**: if the IC-9100 is left in SAT mode, sending the
Sub-select sequence without resetting SAT mode causes TONE to appear on **both Main and Sub**.
SAT MODE OFF (`16 5A 00`) → ON (`16 5A 01`) before the Sub sequence puts the rig in a clean state
so T appears on Sub only.

**Why mode must be re-applied after SAT MODE OFF→ON**: the IC-9100 resets its mode to FM when
SAT MODE is cycled OFF→ON. After the CTCSS CI-V sequence completes, the current DL/UL modes
(stored in `_current_dl_mode` / `_current_ul_mode`) are re-sent via direct CI-V (command `0x06`).

**Why Main TONE OFF is sent explicitly**: SAT MODE OFF→ON can bleed the Sub CTCSS state onto Main.
Sending `16 42 00` to Main after the Sub sequence prevents Main from accidentally displaying T.
...
**`_satmode_exit()`**:
- `rig.send_raw(FE FE civ E0 16 5A 00 FD)` で SAT モードを OFF（CI-V `16 5A`。`set_func` は使わない）
