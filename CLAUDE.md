...
#### Satmode Cold Start 問題（2026-06-19 確認・対策済み）

**問題**: IC-9100/IC-9700 は電源 ON 後（または長時間使用後）の最初の接続で、
`set_func(SATMODE, 1)` の CI-V コマンドを受け取った後もデュアルバンドルーティングの
**ハードウェア初期化に約 1 秒かかる**。この初期化が完了する前に SATMODE OFF → `set_freq(SUB_A)` を
送ると Sub VFO への書き込みがサイレントに無視される（ログにエラーなし）。

アプリ内では `_init_split`（SATMODE ON）から `_satmode_enter`（SATMODE OFF → set_freq）まで
約 300ms しかなく、この Cold Start 状態では必ず UL 書き込みに失敗していた。
モード設定・CTCSS 設定の初回失敗も同じ原因（初期化未完了状態での CI-V）と推定される。

**診断方法**: テストスクリプト `scripts/test_ic9100_satmode_ul.py` を使い、
STEP 1（SATMODE ON）と STEP 2（SATMODE OFF → set_freq(SUB_A)）の間に Enter を押す間隔（1〜2 秒）を
空けると UL 書き込みが成功することで確認。

**現在の対策（2層構造）**:

1. **起動時ウォームアップ** `satmode_warmup()`（`HamlibDirectController`）:
   - アプリ起動直後に `_start_satmode_warmup()`（`MainWindow`）がバックグラウンドスレッドで自動実行
   - 一時的な Hamlib 接続を開いて SATMODE ON CI-V を送り、1.5 秒待機後に閉じる
   - ユーザーが Connect ボタンを押す頃には IC-9100 の初期化が完了している
   - ポートが他プロセスに占有されていた場合はサイレントに無視（`logger.debug` のみ）

2. **Connect 時フォールバック** `_init_split()`（`HamlibDirectController`）:
   - `set_func(SATMODE, 1)` の直後に `time.sleep(1.0)` を挿入
   - 起動時ウォームアップが間に合わなかった場合（起動直後に即 Connect・ポート占有中など）の保険
   - Connect ボタン押下時に 1 秒の待機が発生するが、SATMODE リグのみ・初回接続時のみ

**NET モードへの適用**:
- NET モードでは rigctld → `S 1 Main` → `I {hz}` の経路で UL 書き込みが行われる
- ユーザーが衛星・トランスポンダーを選択してから Connect を押すまでに自然と数秒の間隔が生まれるため
  Cold Start 問題が顕在化していない（2026-06-17 動作確認済み）
- NET モードで Cold Start UL 失敗が報告された場合は `HamlibNetController` にも同様の対策を追加する
