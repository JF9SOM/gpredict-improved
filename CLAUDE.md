...
- **View メニュー構成**（v0.1.0 時点）
  - Language: English（動作）/ Japanese（「To be prepared later.」ダイアログを表示・言語切替は未実装）
  - Time Zone: UTC / Local Time
  - Radar・Pass Chart エントリは削除済み（タブ直接選択で十分なため。Dashboard追加でインデックスがずれた問題を根本解決）
...
6. **SDR フェーズ2（将来）** — HRPT/LRPT 画像・APRS・FT4・gr-satellites・AI-CW・SSTV（設計方針は「SDR 機能設計方針」セクション参照）
7. **SDR Device Installation ダイアログ**（Help メニュー）— USB VID/PID スキャン・ドライバ自動インストール誘導
