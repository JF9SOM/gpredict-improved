...
def get_user_hamlib_dir() -> Path:
    """Return the per-user Hamlib installation directory."""
    try:
...
def is_user_hamlib_installed() -> bool:
    ...
    # Check for the Python binding .so (Linux/macOS) or .pyd (Windows)
    return any(
        (d / f).exists()
        for f in [
            f"_Hamlib.cpython-{sys.version_info.major}{sys.version_info.minor}-x86_64-linux-gnu.so",
            "_Hamlib.pyd",
            f"lib/python{pyver}/site-packages/Hamlib.py",
        ]
    )
...
def windows_asset_name(version: str) -> str:
    """e.g. 'hamlib-w32-4.7.1.zip' (official Hamlib release)"""
    return f"hamlib-w32-{version}.zip"
